...
import type { NerdctlCli } from "./cli";
import {
	extractAddress,
	extractImage,
	extractStatus,
...
export class FirecrackerInstance implements SandboxInstance {
	...
	async info(): Promise<SandboxInfo> {
		...
		const item = parseInspect(r.stdout);
		return {
			...base,
	...
	async getHost(port: number): Promise<string> {
		const r = await this.cli(["inspect", this.id]);
		if (r.exitCode === 0) {
			const ip = extractAddress(parseInspect(r.stdout));
